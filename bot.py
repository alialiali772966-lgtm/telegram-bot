...
def send_welcome(message):
        ...
        if param.startswith("whisper_"):
            try:
                ...
                if sender_id != target_id:
                    bot.send_message(message.chat.id, "❌ هذه الهمسة ليست موجهة لك!")
                    return
    ...
    bot.send_message(message.chat.id, "أهلاً بك! بوت الهمسات جاهز للعمل في المجموعات والتعليقات.")
...
def handle_private_messages(message):
    ...
    if target_info:
        ...
        markup.add(InlineKeyboardButton("👁️ رؤية الهمسة", url=f"https://t.me/{bot_username}?start=whisper_{target_id}_{chat_id}"))
        ...
        bot.send_message(message.chat.id, "الرجاء تحديد شخص لهمسه في المجموعة أو التعليقات أولاً بالرد بكلمة (هـ).")
...
def handle_group_messages(message):
    ...
    markup = InlineKeyboardMarkup()
    bot_username = bot.get_me().username
    markup.add(InlineKeyboardButton("اهمس هنا", url=f"https://t.me/{bot_username}?start=whisper_{target_user_id}_{chat_id}"))
